...
- **D5b 「确认」有两条入口，两条都要记账**：界面上「确认应收」既在**对账单行**上（一键确认一批），
  也在**应收来源/应付条目行**上（逐条确认 + 勾选批量确认）。只在其中一条上记账，
  用户点另一条时看到的仍然是「确认了但银行余额没动」。因此：
  - 逐条确认 → 每条一条流水（来源 `receivable_source` / `supplier_payable_entry`）；
  - 勾选批量确认 → **每条各一条**流水（批量没有单张单据可挂，逐条才追得回去）；
  - 按对账单确认 → 一条按对账单汇总的流水（对账单本身就是一张凭证）。
  几条路径共用 `recordConfirmation` 的「同来源只应有一条流水」，且一条应收/应付一旦被确认
  就不再是草稿，另一条入口的 `status = draft` 条件自然捞不到它 —— 不会重复记账。
  （2026-09-16 更新：界面上原来的「按订单批量确认」表已下线，由勾选批量确认取代
  —— 勾选 + 筛选能精确到某张订单，两套批量机制是多余的；接口 `batch-confirm-by-order` 保留。）
...
- `components/finance/cash-flow-workspace.tsx`：手工流水新增**银行账户**字段与列（与「结算账户」并存，
  注释里写清两者区别）。
- `components/finance/receivable-workspace.tsx` / `payable-workspace.tsx`：创建对账可填**收支项目**；
  「确认应收 / 确认应付」改成弹窗，可补/改**银行账户 + 收支项目**，
  并在响应 `bank_missing` 时明确警告。**逐条确认、勾选批量确认、一键确认（按对账单）三条入口
  都要弹窗收这两个字段**（见 D5b）。
  （2026-09-16 更新：登记收/付款与编辑收付款草稿的入口已下线，见 `finance-batch-confirm-and-copy-trim-2026-09-16.md`。）
