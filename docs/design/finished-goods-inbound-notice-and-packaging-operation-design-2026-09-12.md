...
4. **已完成但还没包装工序的历史单**：可以先「补建包装工序」，再按现有规则把生产单「重新打开」（`completed → in_progress`，有审计）来补录包装工序日报，然后发入库通知；补建后订单进度会把新工序算作未完成，直到包装报工达到目标量——这与「包装完成才能入库」的业务口径一致。工序日报的创建仍沿用既有规则（仅 `in_progress` 可新增，`completed` 只允许更正/删除）。
5. **建单瞬间包装工序占用 1 号位**：新单只有包装工序时它必然是第 1 道；一旦再添加工序（界面走批量接口），包装工序会被顺延到末尾并把 1 号位让出来。因此「单条添加指定 sequence_no=1」在**尚未添加工序的新单**上会返回 409 序号冲突（提示里带占用方），添加工序后即正常；界面不使用该入口（用批量接口）。
6. **不做跨单位换算**：通知数量与订单/工序单位保持一致（沿用现有 Decimal 口径）。
