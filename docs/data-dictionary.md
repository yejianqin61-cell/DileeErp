...
### 3.14 员工花名册字段（人事导入模板口径）

2026-09-16 起，员工档案口径从旧的 8 列模板换成《在职员工花名册》。下表是花名册列到系统字段的落地关系；
花名册里的 5 个公式列（年龄、工龄、当月生日员工、合同即将到期人员、劳务合同即将到期人员）**不落库**，
由后端按当天日期实时派生（F-0226）。

| 编号 | 字段 | 类型 | 上游 | 下游 | 联动 | 状态 |
|---|---|---|---|---|---|---|
| F-0214 | `birth_date` | date | 员工花名册 / 身份证号反推 | 年龄、当月生日、人事台账 | 派生来源 | 已实现 |
| F-0215 | `gender` | enum | 员工花名册 / 身份证号反推 | 人事台账、导出 | 需与身份证自洽 | 已实现 |
| F-0216 | `ethnicity` | string | 员工花名册 | 人事台账、导出 | 引用 | 已实现 |
| F-0217 | `id_card_no` | string | 员工花名册 | 出生日期与性别反推、省市县取名 | 校验位校验；前 6 位查行政区划表（现行 + 历史） | 已实现 |
| F-0218 | `education` | string | 员工花名册 | 人事台账、导出 | 引用 | 已实现 |
| F-0219 | `blood_type` | string | 员工花名册 | 人事台账、导出 | 引用 | 已实现 |
| F-0220 | `social_insurance` | boolean | 员工花名册 | 人事台账、导出 | 引用（仅登记，不自动算薪） | 已实现 |
| F-0221 | `commercial_insurance` | boolean | 员工花名册 | 人事台账、导出 | 引用（仅登记，不自动算薪） | 已实现 |
| F-0222 | `contract_start` / `contract_end` | date | 员工花名册 | 合同到期提醒、人事台账 | 区间单调 | 已实现 |
| F-0223 | `labor_contract_start` / `labor_contract_end` | date | 员工花名册 | 劳务合同到期提醒 | 区间单调 | 已实现 |
| F-0224 | `phone` / `home_address` / `current_address` | string | 员工花名册 | 人事台账、导出 | 引用 | 已实现 |
| F-0225 | `emergency_contact` / `emergency_phone` | string | 员工花名册 | 人事台账、导出 | 引用 | 已实现 |
| F-0226 | `roster_derived_columns`（年龄 / 工龄 / 当月生日员工 / 合同即将到期人员 / 劳务合同即将到期人员） | derived | 出生日期、入职日期、合同结束时间 | 员工目录、员工名单导出 | 实时派生，不落库；合同档位统一为 `正常`/`即将过期`（1 个月内）/`已过期`，另有合并档位 `contract_situation` 取两者中最紧急的一档 | 已实现 |
| F-0227 | `employee_no`（留空自动生成） | string | 花名册导入 / 新建员工 | 日报、工资、考勤 | 自动编码 `EMP-当天日期-序号` | 已实现 |
| F-0228 | 员工 `deleted_at` / `deleted_by` | timestamp / UUID | 员工列表「删除」 | 员工目录、选择器、工资月台账 | 逻辑删除 + 可恢复 | 已实现 |
...
| 人事 | 员工/考勤/绩效 | F-0097~F-0101、F-0187~F-0194、F-0214~F-0225 | 人工录入 / 花名册导入 | 日报、工资 |
| 人事 | 员工花名册导入/导出 | F-0214~F-0228 | Excel《在职员工花名册》/ 员工档案 | 员工目录、员工名单 |
...
| R-0024 | 员工 `id_card_no` | 出生日期 / 性别 / 省市县 | 派生 | 校验位通过时反推出生日期与性别；前 6 位查行政区划表取名（现行表优先，已撤销代码按当时的名称）；与手填值不一致则拒绝整行 |
| R-0025 | 员工 `department_id` | `employee_type` 推断 | 默认值 | 花名册没有员工类型列时，仅当该部门已有员工类型完全一致才照此推断，并在导入结果里报告；不一致则要求手工填写 |
| R-0026 | 员工 `deleted_at` | 员工目录 / 选择器 / 工资月台账 | 逻辑删除 | 删除只写 `deleted_at`，物理行与历史日报/考勤/绩效/台账全部保留；默认列表与导出都不含已删除，`include_deleted=true` 或筛「已删除」才可见并可 `restore` |
...
- 员工「是否缴纳社保/是否缴纳商业险」只是花名册登记的事实字段，**不参与自动算薪**（社保/个税仍按 V1.0 决策人工维护）。
- 身份证住址的省市县由内置行政区划表取名（`apps/api/src/modules/production/china-region.ts` 与
  `apps/web/lib/china-region.ts`，由 `npm run regions:generate` 从现行 + 历史两份公开数据生成）；
  镇/村/门牌一律手填。行政区划会继续调整，表是静态快照，遇到更新的代码时降级为「能查到哪一级用哪一级」。
