...
1. 复制 `.env.example` 为 `.env`，替换数据库密码和初始管理员密码；不要将该文件提交到 Git。
2. 检查编排：`npm run compose:config`。
3. 启动并构建：`npm run compose:up`。
4. 查看状态和日志：`docker compose ps`、`npm run compose:logs`。
5. 在厂内浏览器访问 `http://服务器地址:3000`，健康检查为 `http://服务器地址:3001/api/v1/health`。
6. 停止服务：`npm run compose:down`。升级前先执行备份，升级失败时固定镜像版本并回滚。
