...
export default function LoginPage() {
  ...
  return <main className="login-page"><form className="panel login-panel" onSubmit={submit}><div className="brand"><span className="brand-mark">D</span><div><strong>迪礼 ERP</strong><small>厂内业务系统</small></div></div><h1>登录</h1><label>用户名<Input name="username" required autoComplete="username" /></label><label>密码<Input name="password" type="password" required autoComplete="current-password" /></label>{error && <p className="feedback-error">{error}</p>}<Button type="submit" disabled={loading}>{loading ? "登录中" : "登录"}</Button></form></main>;
}
