...
export default function WorkbenchPage() {
  const [orders, setOrders] = useState<WorkbenchOrder[]>([]); const [selectedOrder, setSelectedOrder] = useState<WorkbenchOrder | null>(null); const [measurements, setMeasurements] = useState<Measurement[]>([]); const [timeline, setTimeline] = useState<AuditEvent[]>([]); const [selectedOrderNo, setSelectedOrderNo] = useState(""); const [filter, setFilter] = useState(""); const [loading, setLoading] = useState(true); const [error, setError] = useState("");
  async function load() {
    setLoading(true); setError("");
    try { const [workbench, measurementRows] = await Promise.all([apiGet<WorkbenchOrder[]>("/order-workbench/orders?page_size=200"), apiGet<Measurement[]>("/production-progress/measurements?page_size=200")]); setOrders(workbench.data); setMeasurements(measurementRows.data); if (selectedOrderNo && !workbench.data.some((order) => order.order_no === selectedOrderNo)) { setSelectedOrderNo(""); setSelectedOrder(null); setTimeline([]); } }
    catch (cause) { setError(messageOf(cause)); } finally { setLoading(false); }
  }
  useEffect(() => { void load(); }, []);
  async function selectOrder(orderNo: string) { setSelectedOrderNo(orderNo); setError(""); try { const [detail, result] = await Promise.all([apiGet<WorkbenchOrder>(`/order-workbench/orders/${encodeURIComponent(orderNo)}`), apiGet<AuditEvent[]>(`/order-workbench/orders/${encodeURIComponent(orderNo)}/timeline`)]); setSelectedOrder(detail.data); setTimeline(result.data); } catch (cause) { setError(messageOf(cause)); setTimeline([]); } }
