...
import { useSessionProfile } from "../lib/session-context";
import { canVisitPath, SURFACE_SECTIONS } from "../lib/surface-permission";
...
type ModuleSummary = { status: string; label: string; counts: { records: number }; source_ids: string[]; missing: boolean; amounts?: { amount: string; currency: string }; quantity_delta?: string; rejected_quantity?: string; quantity?: string; posted_quantity?: string; draft_quantity?: string; notice_count?: number; notified_quantity?: string; pending_inbound_quantity?: string; stock_quantity?: string; defective_stock_quantity?: string; outbound_quantity?: string };
...
export default function WorkbenchPage() {
  ...
  // 工作台上的卡片按表面权限过滤：卡片本身带目标路径，所以"哪些卡片该藏"直接由权限决定，
  // 不用再维护一张"卡片→权限"的映射表。用户拍板：其他角色连工作台上的财务卡片也不该看到。
  const sections = profile?.surface_sections ?? SURFACE_SECTIONS;
  const permittedCards = moduleCards.filter(([, href]) => canVisitPath(sections, href));
  return <>
    ...
    {loading ? <LoadingState label="正在加载订单推进状态" /> : <>
      <section className="panel table-panel"><div className="panel-heading"><h2>订单全链路</h2><span className="panel-note">按订单号汇总，只读</span></div><div className="panel-body"><div className="filter-bar"><label htmlFor="workbench-order-filter">筛选订单<Input id="workbench-order-filter" value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="订单号、状态或阻塞原因" /></label></div><DataTable columns={[orderColumns.accessor("order_no", { header: "订单号" }), orderColumns.accessor("overall_status_label", { header: "总体状态", cell: (info) => <StatusBadge label={info.getValue()} tone={statusTone(info.row.original.overall_status)} /> }), orderColumns.accessor("sales_status", { header: "销售" }), orderColumns.display({ id: "blockers", header: "阻塞原因", cell: (info) => info.row.original.blockers.length ? <div className="workbench-blockers">{info.row.original.blockers.map((item) => <span className="workbench-blocker" title={item.suggestion} key={item.code}>{item.label}</span>)}</div> : <span className="panel-note">无</span> }), orderColumns.accessor("updated_at", { header: "更新时间", cell: (info) => formatBeijingShort(info.getValue() as string) }), orderColumns.display({ id: "actions", header: "操作", cell: (info) => <Button variant="ghost" onClick={() => { void selectOrder(info.row.original.order_no); }}>查看详情</Button> })]} data={visibleOrders} empty={<EmptyState title="暂无订单" description="建立销售单后，这里会显示订单全链路状态。" />} /></div></section>
      {selected && <section className="progress-detail-grid"><section className="panel"><div className="panel-heading"><h2>{selected.order_no} · {selected.overall_status_label}</h2><span className="panel-note">销售状态：{selected.sales_status}</span></div><div className="panel-body"><div className="blocker-list">{selected.blockers.length ? selected.blockers.map((item) => <div className="status-danger progress-warning" key={item.code}><strong>{item.label}</strong><span>{item.suggestion}</span></div>) : <p className="panel-note">当前没有待处理阻塞。</p>}</div></div></section><section className="panel"><div className="panel-heading"><h2>模块状态</h2></div><div className="panel-body module-summary-grid">{permittedCards.map(([label, href, summary]) => <Link className="module-summary-card" href={`${href}?order_no=${encodeURIComponent(selected.order_no)}`} key={label}><strong>{label}</strong><StatusBadge label={summary.label} tone={statusTone(summary.status)} /><span>{summary.missing ? "尚未建立事实" : `${summary.counts.records} 条记录`}</span>{summary.amounts && <span>{summary.amounts.amount} {summary.amounts.currency}</span>}{summary.quantity_delta && <span>库存变动 {summary.quantity_delta}</span>}{summary.rejected_quantity && <span>不合格 {summary.rejected_quantity}</span>}{summary.stock_quantity !== undefined && <span>成品存量 {summary.stock_quantity}</span>}{summary.pending_inbound_quantity !== undefined && <span>待入库 {summary.pending_inbound_quantity}</span>}{summary.defective_stock_quantity !== undefined && Number(summary.defective_stock_quantity) !== 0 && <span>次品 {summary.defective_stock_quantity}</span>}{summary.outbound_quantity !== undefined && <span>已出库 {summary.outbound_quantity}</span>}</Link>)}</div></section><section className="panel" style={{ gridColumn: "1 / -1" }}><div className="panel-heading"><h2>生产计量</h2><span className="panel-note">只读来源汇总</span></div><div className="panel-body"><DataTable columns={[measurementColumns.accessor("operation_name", { header: "工序/来源", cell: (info) => info.getValue() ?? (info.row.original.source_type === "outsource_direct_shipment" ? "外加工直装柜" : "外加工成品回厂") }), measurementColumns.accessor("actual_quantity", { header: "完成数量" }), measurementColumns.accessor("planned_quantity", { header: "计划数量" }), measurementColumns.accessor("difference_quantity", { header: "差额" }), measurementColumns.accessor("over_order_quantity", { header: "超单量" }), measurementColumns.accessor("completion_rate", { header: "完成率", cell: (info) => formatCompletionRate(info.getValue()) }), measurementColumns.accessor("unit", { header: "单位" }), measurementColumns.accessor("status", { header: "计量状态" })]} data={selectedMeasurements} empty={<EmptyState title="暂无计量来源" />} /></div></section></section>}
    </>}
  </>;
}
