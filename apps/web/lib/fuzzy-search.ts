// 模糊搜索（纯函数，无副作用、无 React 依赖）——**全站所有「搜索框」共用这一套匹配语义**。
//
// 使用者：原料仓储情况、以及全部主数据池（供应商池 / 物料池 / 客户池 / 工序池 / 加工地点池 /
// 单位池 / 部门池 / 岗位池 / 银行账户池）。2026-09-16 之前每个池子各写一句
// `xxx.toLowerCase().includes(query.toLowerCase())`：单字查询看不出差别，但「香港 迪礼」
// 这种两词查询在不同池子里的结果会不一致（有的拆词、有的不拆），用户会觉得「这个页面的搜索坏了」。
//
// 为什么单独成模块：搜索匹配规则（拆词、忽略大小写与空白、多字段 OR、词与词 AND）是
// 用户能直接感知的行为，放在页面里就只能靠渲染测试间接覆盖；抽成纯函数后
// `lib/fuzzy-search.test.mjs` 可以直接推演边界（空查询、多词、空白、非字符串字段），
// `lib/pool-search-guard.test.mjs` 再把「每个池页面都接上它」钉住。
//
// 规则：
