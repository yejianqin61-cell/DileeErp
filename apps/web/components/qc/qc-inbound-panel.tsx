...
const labelStatus: Record<string, string> = { draft: "待过账", posted: "已过账", reversed: "已冲销" };
...
export function QcInboundPanel() {
  ...
  const defectiveColumns: ColumnDef<Defective>[] = [
    ...
    { id: "status", header: "状态", cell: ({ row }) => labelStatus[row.original.status] ?? row.original.status },
  ...
  return <section className="panel" style={{ gridColumn: "1 / -1" }}>
    ...
    <div className="panel-body">
      ...
      <div className="action-row"><Button size="sm" variant="secondary" asChild><Link href="/warehouse/finished-goods-storage">去成品仓储情况过账入库</Link></Button></div>
    </div>
    ...
    <div className="panel-body">
      {loading ? <LoadingState /> : <>
        <h3>质检合格待入库</h3>
        <DataTable columns={qcColumns} data={qcAvailable} empty={<EmptyState title="暂无可入库/可登记次品的质检单" description="成品质检录入合格或不合格数量后，这里会出现可入库 / 可登记次品额度。" />} />
        <h3>次品记录</h3>
        <DataTable columns={defectiveColumns} data={defectives} empty={<EmptyState title="暂无次品记录" description="质检不合格数量在上方「登记次品」后在这里过账。" />} />
      </>}
    </div>
  </section>;
}
