...
export function SheetContent({ side = "right", className, children, ...props }: ComponentProps<typeof SheetPrimitive.Content> & { side?: "top" | "right" | "bottom" | "left" }) {
  ...
  return <SheetPrimitive.Portal><SheetPrimitive.Overlay className="ui-dialog-overlay" /><SheetPrimitive.Content className={cn("ui-sheet-content", sideClass, className)} {...props}>{children}<SheetPrimitive.Close className="ui-dialog-close"><X size={16} /><span className="sr-only">关闭</span></SheetPrimitive.Close></SheetPrimitive.Content></SheetPrimitive.Portal>;
}
