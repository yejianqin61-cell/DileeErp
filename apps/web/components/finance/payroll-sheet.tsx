...
export function PayrollSheet<T extends { id: string }>({ columns, rows, onCommit, rowTestId, totalLabel = "合计（当前筛选结果）", empty }: {
  ...
  if (!rows.length) return <>{empty ?? <EmptyState title="本月暂无工资台账" />}</>;
  ...
  return <div className="payroll-sheet">
    ...
    <p className="panel-note" data-testid="payroll-sheet-count">共 {rows.length} 条</p>
  </div>;
}
