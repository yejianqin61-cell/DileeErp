...
export class BomsService {
  ...
  async update(id: string, extensionData: Record<string, unknown>, user: CurrentUser) {
    ...
    if (bom.status === "published") throw new UnprocessableEntityException({ code: "BOM_PUBLISHED_READ_ONLY", message: "已发布 BOM 不可直接编辑，请创建新版本", details: [] });
  ...
  async replaceItems(id: string, items: Array<{ material_id: string; material_snapshot: Record<string, unknown>; required_quantity: string; unit: string; loss_quantity?: string; loss_rate?: string; extension_data?: Record<string, unknown> }>, user: CurrentUser) {
    const bom = await this.get(id);
    if (bom.status === "published") throw new UnprocessableEntityException({ code: "BOM_PUBLISHED_READ_ONLY", message: "已发布 BOM 不可直接编辑，请创建新版本", details: [] });
    if (items.some((item) => !item.material_id || !item.unit || !this.isPositiveDecimal(item.required_quantity) || (item.loss_quantity !== undefined && !this.isNonNegativeDecimal(item.loss_quantity)) || (item.loss_rate !== undefined && !this.isNonNegativeDecimal(item.loss_rate)))) {
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      if (items.length) await tx.bomItem.createMany({ data: items.map((item) => ({ bomId: id, materialId: item.material_id, materialSnapshot: item.material_snapshot as Prisma.InputJsonValue, requiredQuantity: item.required_quantity, unit: item.unit, lossQuantity: item.loss_quantity, lossRate: item.loss_rate, extensionData: (item.extension_data ?? {}) as Prisma.InputJsonValue, ...this.audit.create(user) })) });
    });
