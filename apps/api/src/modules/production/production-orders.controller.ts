...
export class ProductionOrdersController {
  ...
  /** 补建包装（收尾）工序（工序名称含「包装」，已有则幂等返回）。 */
  @Post(":id/packaging-operation") async ensurePackagingOperation(@Param("id") id: string, @CurrentUser() user: CurrentUserType) { return { data: await this.orders.ensurePackagingOperation(id, user), meta: {} }; }
