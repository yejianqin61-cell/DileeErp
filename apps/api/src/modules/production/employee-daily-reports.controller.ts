...
class CreateEmployeeDailyReportDto { @IsUUID() production_order_id!: string; @IsUUID() production_order_operation_id!: string; @IsUUID() employee_id!: string; @IsDateString() report_date!: string; @IsString() wage_mode!: string; @IsString() quantity!: string; @IsOptional() @IsString() duration_minutes?: string; @IsOptional() @IsString() unit_price?: string; @IsOptional() @IsString() @MaxLength(1000) price_override_reason?: string; @IsOptional() @IsString() @MaxLength(1000) remark?: string; }
class UpdateEmployeeDailyReportDto { @IsOptional() @IsDateString() report_date?: string; @IsOptional() @IsString() wage_mode?: string; @IsOptional() @IsString() quantity?: string; @IsOptional() @IsString() duration_minutes?: string; @IsOptional() @IsString() unit_price?: string; @IsOptional() @IsString() @MaxLength(1000) price_override_reason?: string; @IsOptional() @IsString() @MaxLength(1000) remark?: string; @IsString() reason!: string; }
class DeleteEmployeeDailyReportDto { @IsString() reason!: string; }
...
export class EmployeeDailyReportsController {
  ...
  @Delete("production/employee-reports/:id") async remove(@Param("id") id: string, @Body() body: DeleteEmployeeDailyReportDto, @CurrentUser() user: CurrentUserType) { return { data: await this.reports.remove(id, body.reason, user), meta: {} }; }
