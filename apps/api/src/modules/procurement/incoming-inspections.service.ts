...
export class IncomingInspectionsService {
  ...
  async list(orderNo?: string) { const rows = await this.prisma.incomingInspection.findMany({ where: { deletedAt: null, ...(orderNo ? { orderNo } : {}) }, include: { purchaseReceipt: { include: { purchaseOrder: { select: { purchaseOrderNo: true } } } } }, orderBy: { createdAt: "asc" } }); return rows.map((row) => ({ ...row, purchase_order_no: row.purchaseReceipt?.purchaseOrder?.purchaseOrderNo ?? null, batchSequence: Number((row.purchaseReceipt?.extensionData as { batch_sequence?: number } | null)?.batch_sequence ?? (row.extensionData as { batch_sequence?: number } | null)?.batch_sequence ?? 1) })).reverse(); }
