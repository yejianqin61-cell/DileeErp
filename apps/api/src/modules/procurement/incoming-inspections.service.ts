...
export class IncomingInspectionsService {
  ...
  async create(input: { purchase_receipt_id: string; inspected_quantity: string; accepted_quantity: string; conditional_quantity: string; rejected_quantity: string; extension_data?: Record<string, unknown>; remark?: string }, user: CurrentUser) {
    const receipt = await this.prisma.purchaseReceipt.findFirst({ where: { id: input.purchase_receipt_id, deletedAt: null } });
    ...
    if (values.some((value) => !Number.isFinite(value) || value < 0) || values[1] + values[2] + values[3] !== values[0] || values[0] > Number(receipt.quantity)) throw new UnprocessableEntityException({ code: "INSPECTION_QUANTITY_MISMATCH", message: "检验数量分流不合法", details: [] });
