...
export class IncomingInspectionsController { constructor(private readonly inspections: IncomingInspectionsService) {} @Get() async list(@Query("order_no") orderNo?: string) { return { data: await this.inspections.list(orderNo), meta: {} }; } @Post() async create(@Body() body: IncomingInspectionDto, @CurrentUser() user: CurrentUserType) { return { data: await this.inspections.create(body, user), meta: {} }; } @Patch(":id/status") async transition(@Param("id") id: string, @Body() body: TransitionDto, @CurrentUser() user: CurrentUserType) { return { data: await this.inspections.transition(id, body.target, body.reason, user), meta: {} }; } @Post(":id/return") async returnToSupplier(@Param("id") id: string, @Body() body: ReturnDto, @CurrentUser() user: CurrentUserType) { return { data: await this.inspections.returnToSupplier(id, body.reason, user), meta: {} }; } @Patch(":id") async update(@Param("id") id: string, @Body() body: IncomingInspectionUpdateDto, @CurrentUser() user: CurrentUserType) { return { data: await this.inspections.update(id, body, user), meta: {} }; } }
