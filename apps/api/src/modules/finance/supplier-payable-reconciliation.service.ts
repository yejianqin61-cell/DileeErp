...
export class SupplierPayableReconciliationService {
  ...
  async create(input: { supplier_id: string; order_no?: string; purchase_order_id?: string; period_start: string; period_end: string; external_balance: string; currency: string; remark?: string }, user: CurrentUser) {
    ...
    const [entries, payments] = await Promise.all([this.prisma.supplierPayableEntry.findMany({ where }), this.prisma.supplierPayment.findMany({ where: { supplierId: supplier.id, currency: input.currency, deletedAt: null, paymentDate: { gte: start, lte: end }, status: "posted", ...(input.order_no ? { orderNo: input.order_no } : {}) } })]);
    const payable = entries.reduce((sum, row) => sum.plus(row.amount), new Prisma.Decimal(0)); const paid = payments.reduce((sum, row) => sum.plus(row.amount), new Prisma.Decimal(0)); const external = this.decimal(input.external_balance); const difference = payable.minus(paid).minus(external);
