...
export class OrderWorkbenchService {
  ...
  async summary(orderNo: string, order?: Awaited<ReturnType<PrismaService["salesOrder"]["findFirst"]>>) {
    ...
    const [boms, purchases, receipts, inspections, production, inbounds, inventory, qc, finishedInbounds, outbounds, receivables, receivablePayments, payableSources, payables, supplierPayments] = await Promise.all([
      ...
      this.prisma.incomingInspection.findMany({ where: { orderNo, deletedAt: null }, select: { id: true, status: true, inspectedQuantity: true, acceptedQuantity: true, rejectedQuantity: true } }),
