...
test("导出绑定系统领料单：换一张 movement_id 就整表变化（单号/明细/数量都跟着变）", async () => {
  ...
  const sheetA = (await open(await service.exportIssue("movement-A"))).getWorksheet("MI-AAAA");
  const sheetB = (await open(await service.exportIssue("movement-B"))).getWorksheet("MI-BBBB");
...
test("口径差异实证：配料数量随 BOM 变化（活数据），本次领料量随单据变化（单据事实）", async () => {
  ...
  const before = (await open(await service.exportIssue("movement-A"))).getWorksheet("MI-AAAA");
  ...
  const afterBom = (await open(await service.exportIssue("movement-A"))).getWorksheet("MI-AAAA");
  ...
  const afterLine = (await open(await service.exportIssue("movement-A"))).getWorksheet("MI-AAAA");
