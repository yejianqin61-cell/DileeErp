...
test("production.issue_posts_inventory_facts_and_preserves_risk_and_idempotency", async () => {
  ...
  try {
    ...
    await prisma.bomItem.create({ data: { bomId: bom.id, materialId: material.id, unitId: unit.id, materialSnapshot: {}, requiredQuantity: "5", unit: unit.name, ...fixtureAudit.create() } });
