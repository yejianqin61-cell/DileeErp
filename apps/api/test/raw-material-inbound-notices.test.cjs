...
function serviceFor(inspection, created) {
  ...
  const tx = {
    ...
    rawMaterialInboundNotice: {
      create: async ({ data }) => created ?? { id: "notice-1", ...data },
      findFirst: async () => inspection.inboundNotice ?? null,
    },
    rawMaterialInbound: { updateMany: async (input) => { updates.push(input); return { count: 1 }; } }
  };
  const prisma = {
    $transaction: async (callback) => callback(tx),
    rawMaterialInboundNotice: { findFirst: async () => ({ id: "notice-1" }) }
  };
  return { service: new RawMaterialInboundNoticesService(prisma, audit, {}), updates };
}
...
test("inbound notice rejects incomplete or rejected QC", async () => {
  const inspection = {
    ...
    purchaseReceipt: { purchaseOrderId: "po-1", purchaseOrderItemId: "item-1" }, rawMaterialInbounds: [], inboundNotice: null
  };
...
test("inbound notice is idempotent for the same inspection", async () => {
  ...
  const inspection = {
    ...
    purchaseReceipt: { purchaseOrderId: "po-1", purchaseOrderItemId: "item-1" }, rawMaterialInbounds: [], inboundNotice: existing
  };
  const { service } = serviceFor(inspection);
...
test("notice creation links existing draft inbounds without changing stock", async () => {
  const inspection = {
    ...
    purchaseReceipt: { purchaseOrderId: "po-1", purchaseOrderItemId: "item-1" },
    rawMaterialInbounds: [{ quantity: "3", status: "draft" }], inboundNotice: null
  };
