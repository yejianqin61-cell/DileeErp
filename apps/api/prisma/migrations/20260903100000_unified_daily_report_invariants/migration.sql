...
CREATE TEMP TABLE operation_daily_report_ranked AS
...
SET completed_quantity = ranked.total_quantity, updated_at = now()
FROM operation_daily_report_ranked ranked
WHERE r.id = ranked.id AND ranked.rn = 1;
UPDATE operation_daily_reports r
SET deleted_at = now(), updated_at = now()
WHERE r.id IN (SELECT id FROM operation_daily_report_ranked WHERE rn > 1);
DROP TABLE operation_daily_report_ranked;
...
CREATE TEMP TABLE employee_daily_report_ranked AS
...
SET quantity = ranked.total_quantity, duration_minutes = ranked.total_duration,
    calculated_amount = ranked.total_amount, updated_at = now()
FROM employee_daily_report_ranked ranked
WHERE r.id = ranked.id AND ranked.rn = 1;
UPDATE employee_daily_reports r
SET deleted_at = now(), updated_at = now()
WHERE r.id IN (SELECT id FROM employee_daily_report_ranked WHERE rn > 1);
DROP TABLE employee_daily_report_ranked;
