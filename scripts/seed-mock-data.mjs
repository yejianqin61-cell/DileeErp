...
async function main() {
  ...
  const prodA = id(601), prodB = id(602), poa1 = id(603), poa2 = id(604), pob1 = id(605), poa3 = id(606);
  ...
  // 包装工序是每个生产单的收尾工序（按名称识别），厂内成品入库通知基于它的累计报工量。
  await put("productionOrderOperation", poa3, { productionOrderId: prodA, operationCatalogId: opPack, operationNameSnapshot: "包装（模拟）", unitId: u.个, sequenceNo: 3, targetQuantity: "1000", status: "active", ...a });
  ...
  await put("purchaseOrderItem", itemA, { purchaseOrderId: purchaseA, materialId: materialA, materialSnapshot: json({ code: "MOCK-M-001", name: "伞布-蓝色（模拟）" }), unitId: u.码, unitSnapshot: json({ name: "码" }), bomItemId: bomItemA1, supplierId: supplierA, supplierSnapshot: json({ code: "MOCK-S-001", name: "宁波伞布供应商（模拟）" }), expectedDate: day("2026-08-28"), quantity: "1200", unitPrice: "0.80", taxRate: "0", extraFee: "0", amount: "960", extensionData: json({}), ...a });
  await put("purchaseOrderItem", itemB, { purchaseOrderId: purchaseB, materialId: materialA, materialSnapshot: json({ code: "MOCK-M-001", name: "伞布-蓝色（模拟）" }), unitId: u.码, unitSnapshot: json({ name: "码" }), bomItemId: bomItemB1, supplierId: supplierB, supplierSnapshot: json({ code: "MOCK-S-002", name: "苏州加工点（模拟）" }), expectedDate: day("2026-09-01"), quantity: "660", unitPrice: "1", taxRate: "0", extraFee: "0", amount: "660", extensionData: json({}), ...a });
  ...
  const reportA = id(901), reportB = id(902), empReportA = id(903), empReportB = id(904), alert = id(905), packReport = id(911);
  ...
  await put("operationDailyReport", packReport, { idempotencyKey: "MOCK-IDEMP-REPORT-003", productionOrderId: prodA, productionOrderOperationId: poa3, orderNo: "MOCK-ORD-001", productionOrderNoSnapshot: "MOCK-PROD-001", operationNameSnapshot: "包装（模拟）", unitId: u.个, reportDate: day("2026-08-25"), completedQuantity: "420", remark: "包装工序日报（成品入库通知的可通知量来源）", ...a });
  ...
  const submission = id(1101), qc = id(1102), fgInbound = id(1103), defective = id(1104), outbound = id(1105), receivable = id(1106), customerPayment = id(1107), receivableAllocation = id(1108), inboundNotice = id(1112);
  // 厂内成品送检来源是「成品入库通知」（不再用 in_house_completion）：通知由生产按包装工序累计报工量分批发出。
  await put("finishedGoodsInboundNotice", inboundNotice, { noticeNo: "MOCK-FGN-001", orderNo: "MOCK-ORD-001", productionOrderId: prodA, productionOrderOperationId: poa3, productionOrderNoSnapshot: "MOCK-PROD-001", operationNameSnapshot: "包装（模拟）", productNameSnapshot: "蓝色折叠伞", productSpecificationSnapshot: "24骨/晴雨两用", unitId: u.个, unitNameSnapshot: "个", noticeQuantity: "420", noticeDate: day("2026-08-25"), batchNo: "MOCK-B1", status: "partially_inbound", idempotencyKey: "MOCK-IDEMP-FGN-001", remark: "模拟成品入库通知", version: 1, ...a });
  await put("finishedGoodsInspectionSubmission", submission, { submissionNo: "MOCK-FG-QC-001", orderNo: "MOCK-ORD-001", productionOrderId: prodA, sourceType: "finished_goods_inbound_notice", sourceId: inboundNotice, productionOrderNoSnapshot: "MOCK-PROD-001", productNameSnapshot: "蓝色折叠伞", productSpecificationSnapshot: "24骨/晴雨两用", unitId: u.个, unitNameSnapshot: "个", submittedQuantity: "420", submissionDate: day("2026-08-25"), status: "submitted", remark: "成品送检", version: 1, ...a });
  await put("finishedGoodsQcRecord", qc, { qcNo: "MOCK-FG-QC-REC-001", submissionId: submission, orderNo: "MOCK-ORD-001", productionOrderId: prodA, sourceType: "finished_goods_inbound_notice", sourceId: inboundNotice, inspectionDate: day("2026-08-25"), inspectedQuantity: "420", qualifiedQuantity: "410", conditionalAcceptQuantity: "5", rejectedQuantity: "5", conclusion: "conditional_pass", status: "completed", rejectionReason: "5个伞骨划伤", version: 1, remark: "成品 QC", ...a });
